...
A <DNT>**dSYM file**</DNT> is an Xcode project file for <DNT>**debug symbols**</DNT>. It contains the debugging symbols that allow for translation of the initial crash report to human-readable information. This process is known as symbolication.
...
* <DNT>**If the app is running in a simulator:**</DNT> An app running through the simulator cannot be used with crash reporting. The simulator crashes cannot be symbolicated.
* <DNT>**If the debugger is enabled:**</DNT> There can only be one uncaught exception handler registered at a time per application. If running with the debugger attached, New Relic will not capture and report crashes.
* <DNT>**If another crash reporter is enabled:**</DNT> If another uncaught exception handler is registered after New Relic starts, this error message is logged:
