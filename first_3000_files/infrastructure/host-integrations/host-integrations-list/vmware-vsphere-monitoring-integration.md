...
The easiest way to install the integration is to follow our [guided install](https://onenr.io/0Bj3VYmGbRX):

1. Log in to [one.newrelic.com](https://onenr.io/0Bj3VYmGbRX).
2. Click <DNT>**Integrations & Agents**</DNT>, search for "vSphere," and click the integration.
3. Follow the steps in the UI.

If you need to manually install, see the sections below.
...
Data from this service is reported to an [integration dashboard](https://onenr.io/0qwyY4Bzpwn). You can [query this data](/docs/using-new-relic/data/understand-data/query-new-relic-data) for troubleshooting purposes or to create charts and dashboards.
...
<CollapserGroup>
  ...
  >
    One possible reason for data gaps could be because of the integration taking too long to collect and process data from vCenter. In case the integration exceeds the [timeout](/docs/infrastructure/host-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format/#timeout), which by default is `120s`, the infrastructure agent will kill the integration, and a log message like the following will be printed:

    ```shell
    level=warn msg="HeartBeat timeout exceeded after 120000000000" integration_name=nri-vsphere
    ```

    In order to fix this, you could extend the [timeout](/docs/infrastructure/host-integrations/infrastructure-integrations-sdk/specifications/host-integrations-standard-configuration-format/#timeout) parameter in the config file.
