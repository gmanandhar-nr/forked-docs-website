...
<Steps>
  <Step>
    ## Check compatibility and requirements [#req]
  ...
  <Step>
    ...
    To use the NFS integration, you need to first [install the infrastructure agent](/docs/infrastructure/install-infrastructure-agent/get-started/install-infrastructure-agent-new-relic/) on the same host.  
  ...
  <Step>
    ...
    1. Go to <DNT>**[one.newrelic.com > All capabilities](https://one.newrelic.com/all-capabilities) > Infrastructure > Third-party services**</DNT>
    2. Search for <DNT>**NFS integration**</DNT>.
    3. Click on the NFS dashboard.

    When [querying your data](/docs/using-new-relic/data/understand-data/query-new-relic-data), NFS data is attached to the `NFSSample` [event type](/docs/data-apis/understand-data/new-relic-data-types/#event-data).
...
## Disabling the integration [#disable]

Set `metrics_nfs_sample_rate` to `-1` in the infrastructure agent [configuration file](/docs/infrastructure/install-infrastructure-agent/configuration/infrastructure-agent-configuration-settings/#metrics-nfs-sample-rate) to avoid scraping NFS metrics.
