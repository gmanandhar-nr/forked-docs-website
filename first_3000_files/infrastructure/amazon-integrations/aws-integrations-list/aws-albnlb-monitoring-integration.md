...
| `requestCount` | The count of requests processed over IPv4 and IPv6. Requests only increment this metric when the load balancer node was able to select a target. If a request is rejected before a target is choses, it is not reflected.
