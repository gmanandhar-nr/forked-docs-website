...
This admin script will create a connection and report test transaction data under the application <DNT>**Python Agent Test**</DNT> in your account.
...
Avoid using the built-in development servers of any web framework. Also, do not use the OpenShift Python 2.6 cartridge or older Django versions. This is because these development servers are often based on the WSGI server from the <DNT>**wsgiref**</DNT> module from the Python standard library. The WSGI server from the <DNT>**wsgiref**</DNT> module had a bug, which meant it was not fully WSGI-compliant, and this would cause the Python agent to report incorrect data. This issue with the <DNT>**wsgiref**</DNT> module is only fixed in Python 2.7.4. The built-in WSGI server in older versions of Django prior to Django 1.4 had a similar problem.
