...
* <DNT>**The wrapper:**</DNT> The wrapper call is `WSGIApplicationWrapper`. You can use the wrapper in more than one place for distinct WSGI application components that may be stacked. In that case, the first wrapper encountered marks the start of the transaction and the agent determines the target app based on that first wrapper (and ignores subsequent ones).
* <DNT>**The path-based wrapper:**</DNT> The path-based wrapper call is `wrap_wsgi_application`. Use this if you could not reference the WSGI object as a variable in the instrumentation scope. This takes the same parameters as the decorator with additional `module` and `object_path` parameters.
