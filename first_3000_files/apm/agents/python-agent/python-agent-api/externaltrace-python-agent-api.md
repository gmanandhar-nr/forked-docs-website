...
* <DNT>**The context manager**</DNT>: The context manager form is `ExternalTrace`.
* <DNT>**The wrapper**</DNT>: The wrapper form is `ExternalTraceWrapper`. It can be used to return a wrapped function without the use of a decorator.
* <DNT>**The path-based wrapper**</DNT>: The path-based wrapper form is `wrap_external_trace`. This applies the `ExternalTraceWrapper` to a given object through monkey patching. This takes the same parameters as the decorator plus an additional `module` and `object_path` parameter.
