...
* <DNT>**The wrapper:**</DNT> The wrapper call is `ASGIApplicationWrapper`. You can use the wrapper in more than one place for distinct ASGI application components that may be stacked. In that case, the first wrapper encountered marks the start of the transaction and the agent determines the target app based on that first wrapper (and ignores subsequent ones).
* <DNT>**The path-based wrapper:**</DNT> The path-based wrapper call is `wrap_asgi_application`. Use this if you could not reference the ASGI object as a variable in the instrumentation scope. This takes the same parameters as the decorator with additional `module` and `object_path` parameters.
...
| `name`
        ...
        _string_ | Optional, rarely used. Sets a transaction name for all requests captured via the ASGI entry point. Generally not used, because you usually would not want all transactions to have the same name (see also [Metric grouping issue](/docs/agents/manage-apm-agents/troubleshooting/metric-grouping-issues)). |
| `group`
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <DNT>**Required**</DNT>. The ASGI object to be wrapped.
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <DNT>**Required**</DNT>. The module containing the ASGI object.
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        <DNT>**Required**</DNT>. Represents the class method or method for finding the ASGI object within a file.
      </td>
    </tr>
  </tbody>
</table>
