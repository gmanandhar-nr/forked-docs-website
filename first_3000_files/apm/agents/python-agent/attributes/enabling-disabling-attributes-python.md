...
<CollapserGroup>
  ...
  >
    ...
    Turns on or turns off all attributes for <InlinePopover type="browser"/>. This is the data that gets sent to the PageView destination. If `attributes.enabled` is `false`, no attributes will be sent to browser monitoring regardless of how this configuration setting is set.
  </Collapser>
</CollapserGroup>
