...
The `setCurrentRouteName()` API determines the name attributes for [`BrowserInteraction` events](/docs/insights/explore-data/attributes/browser-default-attributes-insights#browserinteraction-attributes), so this API must be called <DNT>**every time**</DNT> there is a route change. This will ensure that `BrowserInteraction` events have the correct attributes.
